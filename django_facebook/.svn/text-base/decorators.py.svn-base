...
def facebook_login_required(view_func=None):
    """
    Decorator which makes the view require an fb login, redirecting
    to the log-in page if necessary.
    ...
    def actual_decorator(view_func):
        ...
        def _wrapped_view(request, *args, **kwargs):
            ...
            #if graph is None:
                logger.info('requesting access with redirect uri: %s', "Yo")
                #response = HttpResponseRedirect("http://www.popcore.me/facebook/connect/?login=1")
                response = HttpResponse("Please login")
                return response
        return _wrapped_view

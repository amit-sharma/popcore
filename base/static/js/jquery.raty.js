...
;(function($) {
	...
	$.fn.raty = function(settings) {
		...
		//AMIT: code by me! Changed the default value of start to the name attribute. So won't work with star widgets that do not have name attribute
		var id			= this.attr('id'),
		...
		if (id == '') {
			//AMIT: code by me! Depends on the body-data being set in the document.ready method
			id_counter = $('body').data('raty_id_counter');
			id = 'raty-' + id_counter;
			$('body').data('raty_id_counter',id_counter+1);
			$global.attr('id', id); 
		}
		...
		if (!options.readOnly) {
			if (options.cancel) {
				...
				$('#' + id + ' img.button-cancel').mouseenter(function() {
				    ...
				    //evt.stopPropagation();
				    //evt.preventDefault();
				    //alert("Yo clicked raty");
				    //console.log("Yo clicked raty");
